...
# TODO : ways to recode modalities in matrix (and to map them)
# TODO : is not great, redo
# recodModalities = function(column, recodMatrix, numericVar=TRUE)
# {
#   copyColumn = matrix(0,nrow(column),1,byrow=T)
#   namesMatrix = names(recodMatrix)
#   
#   for(i in 1:(ncol(recodMatrix)))
#   {
#     nameCol = colnames(recodMatrix)[i]
#     
#     for(j in 1:(nrow(recodMatrix)))
#     {
#       # Replace by "i" or nameCol each value of copyColumn that exists in recodMatrix[,i]
#       if(numericVar)
#         copyColumn[,1][column==recodMatrix[j,i]] = i
#       else
#         copyColumn[,1][column==recodMatrix[j,i]] = nameCol
#     }
#   }
#   
#   return(copyColumn)
